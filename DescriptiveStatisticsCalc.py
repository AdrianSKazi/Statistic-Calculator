...
class PopulationAverage():
    distribution = input("Whether the distribution is normal, if you don't know type in N? (Y/N): ")
    sample = input('Insert sample size: ')
    deviation = input('Whether standard deviation is known? (Y/N): ')

    if distribution.lower() == 'y' and deviation.lower() == 'y':
        def PopulationAverageWithNormalDistributionAndKnownStandardDeviation(self, sigma, n_probki, srednia_probki,
                                                                             wspl_ufnosci, z_alfa):
            min = -((z_alfa * sigma) / math.sqrt(n_probki)) + srednia_probki
            max = (z_alfa * sigma) / math.sqrt(n_probki) + srednia_probki
            return print('P(', round(min, 2), '< m <', round(max, 2), ') z prawdopodobieństwem', wspl_ufnosci * 100,
                         '%')

    elif distribution.lower() == 'y' and deviation.lower() == 'n' and int(sample) < 30:
        def PopulationAverageWithNormalDistributionUnknownStandardDeviationAndSmallSample(self, n_probki,
                                                                                          srednia_probki,
                                                                                          odchylenieStandardoweProbki,
                                                                                          t_alfa_n_minus_1,
                                                                                          wspl_ufnosci):
            min = -(t_alfa_n_minus_1 * odchylenieStandardoweProbki) / math.sqrt(n_probki - 1) + srednia_probki
            max = (t_alfa_n_minus_1 * odchylenieStandardoweProbki) / math.sqrt(n_probki - 1) + srednia_probki
            return print('P(', round(min, 2), '< m <', round(max, 2), ') z prawdopodobieństwem', wspl_ufnosci * 100,
                         '%')
    elif distribution.lower() == 'y' and deviation.lower() == 'n' and int(sample) > 30:
        def PopulationAverageWithNormalDistributionUnknownStandardDeviationAndBigSample(self, n_probki, srednia_probki,
                                                                                        odchylenieStandardoweProbki,
                                                                                        z_alfa, wspl_ufnosci):
            min = -(z_alfa * odchylenieStandardoweProbki) / math.sqrt(n_probki) + srednia_probki
            max = (z_alfa * odchylenieStandardoweProbki) / math.sqrt(n_probki) + srednia_probki
            return print('P(', round(min, 2), '< m <', round(max, 2), ') z prawdopodobieństwem', wspl_ufnosci * 100,
                         '%')
    elif distribution.lower() == 'n' and deviation.lower() == 'n' and int(sample) > 30:
        def PopulationAverageWithUnknownDistrubutionAndBigSample(self, srednia, n_probki, odchylenie_st_probki, z_alfa,
                                                                 wspl_ufnosci):
            min = -(z_alfa * odchylenie_st_probki) / math.sqrt(n_probki) + srednia
            max = (z_alfa * odchylenie_st_probki) / math.sqrt(n_probki) + srednia
            return print('P(', round(min, 2), '< m <', round(max, 2), ') z prawdopodobieństwem', wspl_ufnosci * 100,
                         '%')
    else:
